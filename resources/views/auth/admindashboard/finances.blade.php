...
<div class="card section p-4">
  ...
  <div class="card p-4 mb-4" id="tuition-section">
    <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
      ...
      <input type="text" id="tuitionSearch" class="form-control form-control-sm search-inline" placeholder="Search tuition...">
    </div>
        ...
        <table class="table table-bordered table-striped" id="tuitionTable">
          ...
          @foreach($tuitions as $t)
            ...
            <tr>
              ...
              <td>{{ is_null($t->misc_monthly) ? '—' : number_format((float) $t->misc_monthly, 2) }}</td>
              <td>{{ is_null($t->misc_yearly) ? '—' : number_format((float) $t->misc_yearly, 2) }}</td>
              <td>{{ $t->books_desc ?? '—' }}</td>
              <td>{{ is_null($t->books_amount) ? '—' : number_format((float) $t->books_amount, 2) }}</td>
            ...
            {{-- ensure Tuition::with('optionalFees') in controller --}}
...
<div class="card p-4" id="optional-fees-section">
  <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center gap-2">
      <input type="text" id="feesSearch" class="form-control form-control-sm search-inline" placeholder="Search fees...">
        ...
        <tbody>
          @foreach($optionalFees as $fee)
            ...
            <tr>
              ...
              <td class="text-nowrap">
                <form id="{{ $formId }}" action="{{ route('optionalfees.update', $fee->id) }}" method="POST" class="d-inline">
                  @csrf
                  @method('PUT')
